...
#define R_RX_PL_WID   	0x60     //                                            无线接收寄存器（高字节先输出） 寄存器 
#define RD_RX_PLOAD     0x61  	// 读取接收数据指令  （小丘添加0x61 255位～0位 无线接收寄存器（低字节先输出））
...
void ANO_NRF_Check_Event(void)//NRF收发事件检测
{
	...
	{
		u8 rx_len = ANO_NRF_Read_Reg(R_RX_PL_WID);//数据长度
		...
		if(rx_len<33)
		{
			...
			if(Tx_Rx_mode==0)	//上位机
			{
				Uart1_Put_Buf(NRF24L01_2_RXDATA,rx_len);////24l01收到的数据发送至串口，上位机端使用 
			}
			else  //mode=1 下位机
			{
				delay_ms(1);  //有必要延时，避免PID发送失败 原因未知
			...
			/////////
			ANO_NRF_Write_Reg(FLUSH_RX,0xff);//小丘添加：接收一次数据完成 清空缓冲区，避免传输一段时间就停止接收
			/////////
		}
		else 
		{
